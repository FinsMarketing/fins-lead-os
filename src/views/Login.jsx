...
import { Shield, Users, Briefcase, ChevronRight, Eye, EyeOff } from 'lucide-react'
...
export default function Login({ onLogin, adminPin, team, isMobile }) {
  ...
  const attemptClient = async () => {
    try {
      const clients = await fetchClients()
      const client = clients.find(c => c.loginEmail?.toLowerCase() === email.trim().toLowerCase() && c.loginPassword === password)
      if (client) onLogin('client', client)
      else { setErr('Wrong email or password.'); setPassword('') }
    } catch (e) { setErr('Login failed. Try again.'); console.error(e) }
  }
        ...
        {!mode ? (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { k: 'admin',  Icon: Shield,    title: 'Admin',  sub: 'Fin — full access' },
                { k: 'team',   Icon: Users,     title: 'Team',   sub: 'Sales, marketing, delivery' },
                { k: 'client', Icon: Briefcase, title: 'Client', sub: 'View your dashboards and files' },
              ].map(({ k, Icon, title, sub }) => (
                <button key={k} onClick={() => { setMode(k); setErr(''); setEmail(''); setPassword(''); setPin('') }} style={{
          ...
          <>
            ...
            <div style={{ background: B.surface, border: `1px solid ${B.border}`, borderRadius: 4, padding: 28 }}>
              <div style={{ color: B.bone, fontFamily: FONT_DISPLAY, fontSize: 22, letterSpacing: '0.08em', marginBottom: 4 }}>{mode === 'team' ? 'TEAM' : 'CLIENT'} LOGIN</div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input type='email' value={email} onChange={e => { setEmail(e.target.value); setErr('') }} placeholder='you@company.com' autoFocus
              ...
              <div style={{ marginBottom: err ? 10 : 14, position: 'relative' }}>
                ...
                <input type={show ? 'text' : 'password'} value={password} onChange={e => { setPassword(e.target.value); setErr('') }} onKeyDown={e => e.key === 'Enter' && (mode === 'team' ? attemptTeam() : attemptClient())} placeholder='Password'
              ...
              <Btn onClick={mode === 'team' ? attemptTeam : attemptClient} disabled={!email || !password} full>SIGN IN <ChevronRight size={13} /></Btn>
            </div>
          </>
        )}
      </div>
